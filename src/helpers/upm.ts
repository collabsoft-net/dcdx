...
export const uploadToUPM = async (baseUrl: string, path: string, username?: string, password?: string, verbose = true) =>{
  ...
  if (response) {
    ...
    if (token) {
      ...
      await axios.post(`${baseUrl}/rest/plugins/1.0/?token=${token}`, formData, { headers: {
        ...formData.getHeaders(),
        ...credentials ? { 'Authorization': `Basic ${credentials}`} : {}
      }})
      .then(() => verbose && console.log('Finished uploading plugin archive to UPM'))
      .catch(err => verbose && console.log('Failed to upload plugin archive file to UPM', err));
    } else {
...
export const registerLicense = async (appKey: string, license: string, baseUrl: string, username?: string, password?: string, verbose = true) => {
  const credentials = username && password ? Buffer.from(`${username}:${password}`).toString('base64') : null;
  ...
  if (!rawLicense || rawLicense !== license) {
    ...
    .then(() => verbose && console.log('Finished uploading license to UPM'))
    .catch(err => verbose && console.log('Failed to upload license to UPM', err));
  }
}