...
export const runScalabilityTest = async (stage: TScalabilityTestTypes, options: TAPTScalabilityTestOptions, messages: TAPTTestMessages) => {
  ...
  await persistTestConfiguration(cwd, options.product, baseUrl, duration, true, options.locust, options.force);
